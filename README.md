...
歡迎使用**退休資產、年金與現金流模擬器工具箱**！本專案提供兩個純前端、無須後端（Serverless）的單頁互動式財務規劃工具，旨在幫助使用者評估通漲（Inflation）、資產配置（Asset Allocation）、香港年金（HKMC Annuity）及香港社署長者生活津貼（OALA）對長期「實質購買力」的動態影響。

### 🌐 線上工具體驗 (Live Demos)

1. **📊 工具一：退休資產與實質購買力提領模擬器**
   * **檔名：** `asset_withdrawal_simulator_0.1.html`
   * **定位：** 通用財務模型（適用於全球／跨國退休規劃）。
   * **核心功能：** 評估股票與債券資產組合，在考慮預期通脤率與購買力折損下，動態模擬提領金額演變與 4% 提領法則（4% Rule）的安全邊界。

2. **🇭🇰 工具二：香港年金 + 長生津與實質購買力模擬器 (2026)**
   * **檔名：** `hkmc_oala_simulator_2026_0.1.html`
...
Welcome to the **Retirement Wealth, Annuity & Cash Flow Simulators Portal**! This repository hosts two pure client-side, interactive web tools designed to help individuals evaluate the long-term impact of inflation, asset allocation, public/private annuities, and government welfare subsidies on real purchasing power during retirement.

### 🌐 Interactive Tools

1. **📊 Simulator 1: Retirement Asset & Purchasing Power Drawdown Simulator**
   * **Filename:** `asset_withdrawal_simulator_0.1.html`
   * **Scope:** Universal financial model for general retirement planning.
...
2. **🇭🇰 Simulator 2: HKMC Annuity + OALA Cash Flow Simulator (2026)**
   * **Filename:** `hkmc_oala_simulator_2026_0.1.html`
...
├── asset_withdrawal_simulator_0.1.html   # 工具一：通用提領模擬器 / Simulator 1
├── hkmc_oala_simulator_2026_0.1.html    # 工具二：香港年金長生津模擬器 / Simulator 2
